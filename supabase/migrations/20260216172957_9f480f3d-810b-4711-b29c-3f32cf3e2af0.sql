...
-- Role enum for future role-based access
...
CREATE POLICY "SEO data is public"
...
INSERT INTO storage.buckets (id, name, public) VALUES ('seo-images', 'seo-images', true);
...
CREATE POLICY "SEO images are public" ON storage.objects
  FOR SELECT USING (bucket_id = 'seo-images');

CREATE POLICY "Admins can upload SEO images" ON storage.objects
  FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'seo-images' AND public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can delete SEO images" ON storage.objects
  FOR DELETE TO authenticated
  USING (bucket_id = 'seo-images' AND public.has_role(auth.uid(), 'admin'));

-- Seed default page SEO entries
INSERT INTO public.page_seo (page_slug, page_name) VALUES
  ('/', 'Home'),
  ('/pench-safari', 'Pench Safari'),
  ('/kanha-safari', 'Kanha Safari'),
